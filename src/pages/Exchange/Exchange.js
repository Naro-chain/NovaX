...
export const Exchange = forwardRef((props, ref) => {
  ...
  const renderChart = () => {
    // Check if there's trading data or if user is connected
    const hasTradingData = positions && positions.length > 0;
    const hasOrders = orders && orders.length > 0;
    const hasTokenData = infoTokens && Object.keys(infoTokens).length > 0;
    
    // Show real-time graph if no trading data or user not connected
    if (!active || (!hasTradingData && !hasOrders && !hasTokenData)) {
      return (
        ...
        <div className="Exchange-right">
          <div className="trade-panel">
            <div className="trade-toggle">
              <button className="trade-toggle-btn active">Buy</button>
              <button className="trade-toggle-btn">Sell</button>
            </div>
            <SwapBox
              pendingPositions={pendingPositions}
              setPendingPositions={setPendingPositions}
              ...
              minExecutionFeeErrorMessage={minExecutionFeeErrorMessage}
            />
            <div className="trade-time-sales">
              <div className="trade-time-sales-title">Time & Sales</div>
              {/* Placeholder for time & sales table, can be replaced with real data */}
              <table className="trade-time-sales-table">
                <thead>
                  <tr>
                    <th>Time</th>
                    <th>Price</th>
